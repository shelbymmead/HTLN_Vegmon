...
year <- unique(na.omit(canopy$Year))

supplement <- overstory1 |>
  select(year = Year2, site) |>
  distinct() |>
  full_join((crossing(year, class)), by = "year")|>
  arrange(year, site, class)

supplement[,c("site", "class")] <- sapply(supplement[,c("site", "class")], as.character) #setting site and class as characters
supplement <- tbl_df(supplement)
...
current <- max(year) #sets "current" as value representing most recent yr of sampling
first <- min(year) #sets "first" as value representing first yr of sampling
...
ba_site_class2 <- left_join(supplement, ba_site_class1, by = c('year' = 'Year2', 'site', 'class')) #joining with supplement table to populate all combos for site, class
ba_site_class2$ba_site[is.na(ba_site_class2$ba_site)] <- 0
ba_site_class3 <- left_join(ba_site_class2, nsites, by = c("year" = "Year2"))

if (min(nsites$nsites) >= 7 ) { #if sites are >= 7, the CI values will be bootstrapped
...
ba_site_class2_D <- left_join(supplement, ba_site_class1_D, by = c('year' = 'Year2', 'site', 'class')) #joining with supplement table to populate all combos for site, class
ba_site_class2_D$ba_site[is.na(ba_site_class2_D$ba_site)] <- 0
ba_site_class3_D <- left_join(ba_site_class2_D, nsites, by = c("year" = "Year2"))
...
density2 <- left_join(density1, nsites, by = c("year" = "Year2"))

 # Iterate across year and class to get bootstrapped confidence intervals of ba
if (min(nsites$nsites) >= 7 ) { #this runs if sites are >7
density_year_class <- 
  purrr::map2(yxc_df$year, yxc_df$class, # iterate on each year x class combination 
              function(yr, cls){
                # create dataset for a given year x class combination
...
density2_D <- left_join(density1_D, nsites, by = c("year" = "Year2"))
...
species_ba_site <- left_join(species_ba_site, nsites, by = c("year" = "Year2"))
...
species_density_site <- left_join(species_density_site, nsites, by = c("year" = "Year2"))
...
canopy_site2 <- left_join(canopy_site, nsites, by = c("year" = "Year2"))
  
 
if (min(nsites$nsites) >= 7 ) {
# Bootstrap function that will be used in purrr
...
type_regen2 <- left_join(type_regen1, nsites, by = c("year" = "Year2"))
...
regen_species1 <- regeneration1 |>
  ...
  select(!count)|>
  left_join(nsites, by = c("year" = "Year2"))|>
