...
# LIBO 2011-2024  Overstory, Regen, Canopy Analysis
...
YearBA <- ggplot(BA_Year)+ #makes a plot using BA_Year dataset we just created
 ...
 labs(y = bquote('Mean Basal Area ' ~ (m^2/ha)))+
  ...
  theme
YearBA
ggsave('BasalArea.jpg', dpi = 600, path = "Output Graphs") #saves basal area graph to Output Graphs
...
BA_by_class <- ggplot(BA_Year_Class, aes(x = Class, y =BA_Year_Class, fill=Year))+ #makes plot using our BA_Year_Class dataset, color by class
  ...
  theme
BA_by_class
ggsave('BasalArea_by_class.jpg', dpi = 600, path= "Output Graphs", height =5)#saves basal area by class graph to Output Graphs
```
...
YearBA_D <- ggplot(BA_Year_D)+ #makes a plot using BA_Year dataset we just created
 ...
 labs(y = bquote('Mean Basal Area ' ~ (m^2/ha)))+
  ...
  theme
YearBA_D
ggsave('BasalArea_D.jpg', dpi = 600, path = "Output Graphs") #saves basal area graph to Output Graphs
...
BA_by_class_D <- ggplot(BA_Year_Class_D, aes(x = Class, y =BA_Year_Class, fill=Year))+ #makes plot using our BA_Year_Class dataset, color by class
  ...
  theme
BA_by_class_D
ggsave('BasalArea_by_class_D.jpg', dpi = 600, path= "Output Graphs", height =5)#saves basal area by class graph to Output Graphs
```
...
YearDensity <- ggplot(Density_Year, aes(x = Year, y = StemDensity_Year))+ #make ggplot of average density each year, using Density_Year table we just made
  ...
  theme
YearDensity
ggsave('Density.jpg', dpi = 600, path = "Output Graphs")
...
ClassDensity
ggsave('Density_Class.jpg', dpi = 600, path = "Output Graphs")
```
...
YearDensity_D
ggsave('Density_D.jpg', dpi = 600, path = "Output Graphs")
...
ClassDensity_D
ggsave('Density_Class_D.jpg', dpi = 600, path = "Output Graphs")
```
...
This chunk creates occurance table
```{r}
...
Overstory %>%
  ...
  write.csv("Output Tables\\PercentOccurence.csv", row.names = F)
...
ggsave('Species_BA_Change_subtraction.jpg', dpi = 600, height = 8, width = 8, path = "Output Graphs")
...
ggplot(PivotSpecies_Density, aes(y = reorder(AcceptedSpecies, desc(AcceptedSpecies)), x = Change, fill = Direction))+ # sets ggplot to show species change in density
  ...
  geom_vline(xintercept = 0)
ggsave('Species_Density_Change_subtraction.jpg', dpi = 600, height = 8, width = 8, path = "Output Graphs")
```
...
Mean_Canopy <- ggplot(Canopy2)+ ## Makes ggplot for year canopy mean
  ...
  theme
Mean_Canopy
ggsave("Mean_Canopy.jpg", dpi = 600, path = "Output Graphs", height = 4)
...
Canopy_by_site<- ggplot(Canopy1, aes(x = Site, y =SiteMean, fill=Year))+ # makes ggplot of canopy with color indicating site
  ...
  theme
Canopy_by_site
ggsave("Canopy_by_site.jpg", dpi = 600, path = "Output Graphs", height = 4)
```
...
TypeRegen_graph
ggsave('Regeneratoin_Type.jpg', path = "Output Graphs", dpi= 600)
...
RegenSpecies %>%
  ...
  theme(axis.text.y = element_text(face = "italic"), #makes species names italic
        axis.text.x = element_text(angle = 90))+
  geom_vline(xintercept = 0)
ggsave('Regeneration_Species_Seedling_subtraction.jpg', path = "Output Graphs", dpi= 600, height = 6)
...
RegenSpecies %>%
  ...
  theme(axis.text.y = element_text(face = "italic"),#makes species names italic
        axis.text.x = element_text(angle = 90))+
  geom_vline(xintercept = 0)
ggsave('Regeneration_Species_SmallSap_subtraction.jpg', path = "Output Graphs", dpi= 600, height = 6)
...
RegenSpecies %>%
  ...
  theme(axis.text.y = element_text(face = "italic"))+#makes species names italic
  geom_vline(xintercept = 0)
ggsave('Regeneration_Species_LargeSap_subtraction.jpg', path = "Output Graphs", dpi= 600, height = 6)
